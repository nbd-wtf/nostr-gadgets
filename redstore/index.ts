...
export class RedEventStore {
   ...
   */
  public async getOutboxBounds(): Promise<{ [pubkey: string]: { [kind: number]: [start: number, end: number] } }> {
    if (!this.#fullyInitialized) await this.init()
   ...
   * only for use by the outbox module.
   */
  public async setOutboxBound(pubkey: string, kind: number, bound: [start: number, end: number]): Promise<void> {
    if (!this.#fullyInitialized) await this.init()
